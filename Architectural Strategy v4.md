...
| **State Consistency** | **Challenging:** Requires passing state across a network boundary. Both modes must produce equivalent outputs into the same planner/task state schema — see §6 Compatibility Contract. | **Native:** Uses a single, unified `AgentState` throughout the task. No serialization boundary. |
